...
//输出流，向客户端写信息
        ...
        try {
            String read = br.readLine();
            {
                ...
                switch (server_type) {
                    ...
                    case 1: {
                        pw.println(sign_deal(read));
                        break;
                    }
                    case 2: {
                        pw.println(getPapers_Deal());
                        break;
                    }
                    //3获得卷头
                    case 3: {
                        pw.println(getPaper_head(Integer.parseInt(StringDeal.queryString(read, "#paper_id="))));
                        break;
                    }
                    //4获得卷体
                    case 4: {
                        pw.println(getPaper_inner(Integer.parseInt(StringDeal.queryString(read, "#paper_id="))));
                        break;
                    }
                    //5是试卷提交
                    case 5: {
                        pw.println(summit_Ans(read));
                        break;
                    }
                    //获取题目
                    case 6: {
                        pw.println(request_questions());
                        break;
                    }
                    //删除题目
                    case 7: {
    ...
    public static String insertgrade(String s) {
        String re = "";
        String blank_grade =StringDeal.queryString(s,"#code=");
        try {
            Connection conn = Driver.getConn();
            String sql = "insert into paper_ans(blank_grade,choice_grade) values(?,?) ";
            int totalgrade=Server_Deal.getchoicegrade()+Integer.parseInt(blank_grade);

            PreparedStatement preSql;
            preSql = conn.prepareStatement(sql);
            preSql.setString(1, blank_grade);
          preSql.setString(2, String.valueOf(totalgrade));
            re = "ok";
            preSql.close();
            conn.close();
    ...
    public static int getchoicegrade(){
        int x=0;
        try{
            Connection conn = Driver.getConn();
            String sql = "select * from where stu_id=1234 ";
            PreparedStatement preSql;
            preSql = conn.prepareStatement(sql);
            ResultSet rs=preSql.executeQuery();
            while(rs.next()){
                x=(int)rs.getInt("choice_grade");
            }
        }catch (SQLException | NullPointerException e) {
            System.out.println(e);
        }
